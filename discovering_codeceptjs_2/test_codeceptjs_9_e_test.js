...
#with the stepByStepReport images + allure reports
npx codeceptjs run --plugins stepByStepReport --plugins allure --steps test_codeceptjs_9_e_test.js


npx codeceptjs run --steps --grep "@post" test_codeceptjs_9_e_test.js
npx codeceptjs run --steps --verbose --grep "@post" test_codeceptjs_9_e_test.js
...
Scenario('Check WP Backoffice Journalist profile creation', async (I, globalData, uiElement, pluginJournalist) => {
...
Scenario('Check WP Frontoffice the baseline', async (I) => {
...
Scenario('Debug :: ensure I can debug some tricky part ', async (I, extraData) => {
    ...
    /*  Languages & Expertises */
    pluginJournalist.JOURNALIST_LANGUAGES_CHECKLIST.forEach((locator, index) => {
        I.say('pluginJournalist language => '+locator+' '+index+'');
    });

    pluginJournalist.JOURNALIST_EXPERTISES_CHECKLIST.forEach((locator, index) => {
        I.say('pluginJournalist expertise => '+locator+' '+index+'');
    });
